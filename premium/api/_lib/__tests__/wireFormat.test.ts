...
describe('buildJobData / parseJobData', () => {
	...
	it('parses job data produced under the current version', () => {
		const payload = { executionId: EXECUTION_ID, kind: 'reconcile' };
		expect(parseJobData(buildJobData(payload))).toEqual({
			wireVersion: 1,
			payload,
		});
	});
	...
	it('rejects job data from another wire version', () => {
		expect(() => parseJobData({ wireVersion: 0, payload: {} })).toThrow(
			WireFormatError,
		);
		expect(() => parseJobData({ wireVersion: 0, payload: {} })).toThrow(
			/unsupported wire format version 0/,
		);
		expect(() => parseJobData({ wireVersion: '1', payload: {} })).toThrow(
			/unsupported wire format version 1/,
		);
	});
...
describe('parseExecutionResult — success', () => {
	...
	it('rejects a report-export result without an artifact', () => {
		expect(() =>
			parseExecutionResult(validResult({ artifact: undefined })),
		).toThrow(/result.artifact must be an object for report-export results/);
	});
...
describe('parseExecutionResult — failure', () => {
	...
	it('parses a non-retryable failure', () => {
		const result = parseExecutionResult({
			...
			error: {
				code: 'BAD_SCOPE',
				message: 'window outside retention',
				retryable: false,
			},
		});
		expect(
			(result as Extract<ExecutionResult, { status: 'failed' }>).error
				.retryable,
		).toBe(false);
	});
...
describe('parseExecutionResult — envelope fields', () => {
	it('rejects non-object results', () => {
		expect(() => parseExecutionResult(null)).toThrow(
			/result must be an object/,
		);
		expect(() => parseExecutionResult(['result'])).toThrow(
			/result must be an object/,
		);
	});
	...
	it('rejects a missing executionId', () => {
		...
		expect(() => parseExecutionResult(rest)).toThrow(
	...
	it('rejects a non-UUID-v4 executionId', () => {
		expect(() =>
			parseExecutionResult(validResult({ executionId: 'not-a-uuid' })),
		).toThrow(/result.executionId must be a UUID v4/);
		expect(() =>
			parseExecutionResult(
				validResult({ executionId: '9c1f2e8a-6d4b-1f0e-9a2c-3b7d5e8f1a02' }),
			),
		).toThrow(/result.executionId must be a UUID v4/);
	});
	...
	it('rejects an unknown kind', () => {
		expect(() =>
			parseExecutionResult(validResult({ kind: 'backfill' })),
		).toThrow(/result.kind must be one of: reconcile, report-export/);
	});

	it('rejects an unknown status', () => {
		expect(() =>
			parseExecutionResult(validResult({ status: 'running' })),
		).toThrow(/result.status must be "success" or "failed"/);
	});

	it('rejects an impossible completedAt timestamp', () => {
		expect(() =>
			parseExecutionResult(
				validResult({ completedAt: '2026-02-30T18:10:00Z' }),
			),
		).toThrow(/result.completedAt must be a real ISO-8601 UTC timestamp/);
	});

	it('rejects a non-UTC completedAt timestamp', () => {
		expect(() =>
			parseExecutionResult(
				validResult({ completedAt: '2026-07-31T18:10:00+01:00' }),
			),
		).toThrow(/result.completedAt must be a real ISO-8601 UTC timestamp/);
	});
...
describe('parseExecutionResult — artifact rules', () => {
		...
		expect(() =>
			parseExecutionResult(
				validResult({
					artifact: {
						...ARTIFACT,
						reportUrl: 'http://files.hoursmith.dev/r.csv',
					},
				}),
			),
		).toThrow(/result.artifact.reportUrl must be an absolute https URL/);
		expect(() =>
			parseExecutionResult(
				validResult({ artifact: { ...ARTIFACT, reportUrl: 'not a url' } }),
			),
		).toThrow(/result.artifact.reportUrl must be an absolute https URL/);
	});
	...
	it('rejects an empty or oversized fileName', () => {
		expect(() =>
			parseExecutionResult(
				validResult({ artifact: { ...ARTIFACT, fileName: '' } }),
			),
		).toThrow(/result.artifact.fileName must be a non-empty string/);
		expect(() =>
			parseExecutionResult(
				validResult({ artifact: { ...ARTIFACT, fileName: 'x'.repeat(256) } }),
			),
		).toThrow(/result.artifact.fileName must be a non-empty string/);
	});
	...
	it('rejects an empty contentType', () => {
		expect(() =>
			parseExecutionResult(
				validResult({ artifact: { ...ARTIFACT, contentType: '' } }),
			),
		).toThrow(/result.artifact.contentType must be a non-empty string/);
	});

	it('rejects a negative or fractional byteSize', () => {
		expect(() =>
			parseExecutionResult(
				validResult({ artifact: { ...ARTIFACT, byteSize: -1 } }),
			),
		).toThrow(/result.artifact.byteSize must be a non-negative integer/);
		expect(() =>
			parseExecutionResult(
				validResult({ artifact: { ...ARTIFACT, byteSize: 1.5 } }),
			),
		).toThrow(/result.artifact.byteSize must be a non-negative integer/);
		expect(() =>
			parseExecutionResult(
				validResult({ artifact: { ...ARTIFACT, byteSize: '12345' } }),
			),
		).toThrow(/result.artifact.byteSize must be a non-negative integer/);
	});
});
...
describe('parseExecutionResult — error rules', () => {
	...
	it('rejects a malformed error code', () => {
		expect(() =>
			parseExecutionResult({
				...FAILED,
				error: { ...FAILED.error, code: 'jira_api_error' },
			}),
		).toThrow(/result.error.code must be SCREAMING_SNAKE/);
		expect(() =>
			parseExecutionResult({
				...FAILED,
				error: { ...FAILED.error, code: 'HAS SPACE' },
			}),
		).toThrow(/result.error.code must be SCREAMING_SNAKE/);
	});
	...
	it('rejects an empty or oversized error message', () => {
		...
		expect(() =>
			parseExecutionResult({
				...FAILED,
				error: { ...FAILED.error, message: 'x'.repeat(2_001) },
			}),
		).toThrow(/result.error.message must be a non-empty string/);
	});
	...
	it('rejects a non-boolean retryable flag', () => {
		expect(() =>
			parseExecutionResult({
				...FAILED,
				error: { ...FAILED.error, retryable: 'yes' },
			}),
		).toThrow(/result.error.retryable must be a boolean/);
	});
});
