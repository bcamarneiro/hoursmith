 ...
 * Combined with the root `middleware.ts` (which handles CORS + content-type
 * gates), this gives full-stack request hygiene without duplicating
 * parse/validate logic across every route file.
 *
 * Design:
 ...
 * });
 * if (!result.ok) return jsonError(result.status, { error: result.error });
